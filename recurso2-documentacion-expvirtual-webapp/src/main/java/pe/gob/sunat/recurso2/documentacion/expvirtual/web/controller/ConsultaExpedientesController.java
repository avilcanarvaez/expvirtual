...
public class ConsultaExpedientesController extends MultiActionController {
	...
	try {
		...
		// Fin [jquispe 27/05/2016]
			...
			if (Utils.isEmpty(listaExpedientesVirtuales)) {
				if(!flagCargaInicial){
					modelAndView.addObject("msjError",AvisoConstantes.EXCEP_MODULO_04_01_016);
				}else{
		...
		if (Utils.isEmpty(listaExpedientesVirtuales)) {
			modelAndView.addObject("msjError",AvisoConstantes.EXCEP_MODULO_04_01_012);
			// Seguimiento CC	- RESULTADO DE LA ACCION ERROR
			beanSeguiCC = new HashMap<String, Object>();
			beanSeguiCC.put("num_expedv", ValidaConstantes.SEPARADOR_GUION);
			beanSeguiCC.put("cod_tipseguim", ValidaConstantes.IND_TIP_SEG_CC);
	       ...
	       estiloCelda.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);	       
	        ...
	        estiloRecorrido.setTopBorderColor((short) 8);
	         for (int i = 0; i < listadoExportar.size(); i++) {
	           ...
	           String numAcumulador = listadoExportar.get(i).get("numExpedienteAcumulador").toString().trim();
	           ...
	           // Inicio [jquispe 27/05/2016] Se modifico para agregar la nueva columna Indicador de Acumulacion
	           celda3.setCellValue(listadoExportar.get(i).get("desIndicadorAcumulado").toString().trim());
	           // Fin [jquispe 27/05/2016]
	           ...
	           // Inicio [jquispe 27/05/2016] Se modifico para agregar la nueva columna Indicador de Acumulacion
	           celda4.setCellValue(Utils.convertirUnicode(listadoExportar.get(i).get("desProceso").toString().trim()));
	           // Fin [jquispe 27/05/2016]
	           ...
	           // Inicio [jquispe 27/05/2016] Se modifico para agregar la nueva columna Indicador de Acumulacion
	           celda5.setCellValue(Utils.convertirUnicode(listadoExportar.get(i).get("desTipoExpediente").toString().trim()));
	           // Fin [jquispe 27/05/2016]
	           ...
	           // Inicio [jquispe 27/05/2016] Se modifico para agregar la nueva columna Indicador de Acumulacion
	           celda6.setCellValue(listadoExportar.get(i).get("desEstado").toString().trim());
	           // Fin [jquispe 27/05/2016]
	           ...
	           celda7.setCellValue(listadoExportar.get(i).get("fechaDocumentoOrigen").toString().trim());
 ...
 @RequestMapping(value = { "/cargarDocumentosExpOrigen" }, method = { RequestMethod.GET })
	public ModelAndView cargarDocumentosExpOrigen(HttpServletRequest request,
	...
	try {
		...
		if (t6614ExpVirtualBean != null) {
			 ...
			 modelAndView.addObject("fechaDocOrigen",new JsonSerializer().serialize(fechaDocOrigen));
			 modelAndView.addObject("fechaRegExpediente",new JsonSerializer().serialize(fechaRegExpediente));
			 modelAndView.addObject("razonSocial",new JsonSerializer().serialize(beanContribuyente.getDesRazonSocial()));
			 modelAndView.addObject("t6614ExpVirtualBean",new JsonSerializer().serialize(t6614ExpVirtualBean));
			 modelAndView.addObject("seguimientoBean",new JsonSerializer().serialize(seguimientoBean));		 
		}	
		...
		modelAndView.addObject("listaDocumentos",new JsonSerializer().serialize(listaDocumentos));
	...
	public ModelAndView verificarListaDocumentoExpediente(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			modelAndView.addObject("esVacioListaDocumentos", esVacioListaDocumentos);
	...
	public ModelAndView cargarDocumentosExpOrigenAcum(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView modelAndView;
...
@RequestMapping(value = "/descargarArchivoDoc", method = RequestMethod.GET,produces=MediaType.APPLICATION_JSON_VALUE + ";charset=utf-8")
	...
	public void exportarExcelDoc(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			for (int i = 0; i < listadoExportar.size(); i++) {
				...
				texto = new HSSFRichTextString(String.valueOf(i));
				celda.setCellValue(texto.toString());
	...
	@RequestMapping(value = { "/cargarArchivosExpVirtual" }, method = { RequestMethod.GET })
	public ModelAndView cargarArchivosExpVirtual(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (log.isDebugEnabled()) log.debug("Error - ConsultaExpedienteControler.cargarArchivosExpVirtual");
			...
			if (log.isDebugEnabled()) log.debug("Final - ConsultaExpedienteControler.cargarArchivosExpVirtual");
	...
	public ModelAndView validarExtensionArchivo(HttpServletRequest request, HttpServletResponse response) {
...
//Inicio [jquispe 30/05/2016]
public void setIndiceExpedienteVirtualService(
		IndiceExpedienteVirtualService indiceExpedienteVirtualService) {
