...
public class OtrosEscritosElecronicosController extends MultiActionController {
	...
	public ModelAndView inicioConsultaOtrosEscritosEView(HttpServletRequest request, HttpServletResponse response) {
	   ...
	   ModelAndView modelAndView;
		...
		if (log.isDebugEnabled())log.info( "Inicio - OtrosEscritosElecronicosController.inicioConsultaOtrosEscritosEView");
	...
	public ModelAndView cargarDatosBusquedaSession(HttpServletRequest request, HttpServletResponse response) {
		...
		if (log.isDebugEnabled())log.info((Object) "Inicio - OtrosEscritosElecronicosController.cargarDatosBusquedaSession");
	...
	public ModelAndView removerDatosBusquedaSession(HttpServletRequest request, HttpServletResponse response) {
		...
		if (log.isDebugEnabled())log.info((Object) "Inicio - OtrosEscritosElecronicosController.removerDatosBusquedaSession");
		...
		try {
   		...
   		hoja.addMergedRegion(new Region(1, (short) 0, 1, (short) 7));
			...
			for (int i = 0; i < listadoExportar.size(); i++) {
				...
				celda4.setCellValue(Utils.toStr(listadoExportar.get(i).get("numExpedienteVirtual") == null ? ValidaConstantes.CADENA_VACIA : Utils.convertirUnicode(listadoExportar.get(i).get("numExpedienteVirtual").toString().trim())));
   ...
   public void descargarEscrito(HttpServletRequest request,HttpServletResponse response) {
      ...
      if (log.isDebugEnabled()) log.info( "Inicio - OtrosEscritosElecronicosController.descargarEscrito");
   ...
   public ModelAndView cargarListadoExpedientesPendientes(HttpServletRequest request, HttpServletResponse response) {
      ...
      if (log.isDebugEnabled())log.info((Object) "Inicio - OtrosEscritosElecronicosController.cargarListadoExpedientesPendientes");
      ...
      try {
         ...
         if (!Utils.isEmpty(numExp)) {
            listaExpedientesVirtuales = null;  //expedienteVirtualService.obtenerListaExpedienteVirtual(mapParametrosBusqueda);
