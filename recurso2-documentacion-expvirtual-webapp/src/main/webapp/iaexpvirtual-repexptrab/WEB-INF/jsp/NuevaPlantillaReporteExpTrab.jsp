...
hr {
 ...
 <script type="text/javascript">
	...
	var titulos =  ${titulos};
	var flagPrevisualizacion=false; // Inicio [gangles 26/05/2016] Flag para la funcionalidad de la previsualización de la plantilla a crear // Fin [gangles 26/05/2016]
	...
	$(function () {
		...
		/*$("#dessPlanReporte").keypress(function(e){
		...
		$('#frmPlantillas').bootstrapValidator({
            ...
            e.preventDefault();
            if(flagPrevisualizacion==false){// Inicio [gangles 29/05/2016] Se agrega la opción de marcar y descarcar todos los chekbox
            	nuevaPlantilla();
            }else{
            	previsualizarPlantilla();
            }	// Fin [gangles 29/05/2016] 		
    	});
		
		//Inicio [gangles 29/05/2016] Se agrega la opción de marcar y descarcar todos los chekbox		
		jQuery('#selectAllDocumentos').prop('disabled', true);
		jQuery('#selectAllReportes').prop('disabled', true);
	...
	function construirTablaDocumentos(dataGrilla) {
		...
		$('#tblDocumentos').DataTable({
			...
			columns : [
				{data : "numOrden", sClass: 'centered'},			
				{data : "codTipoDocumento", sClass: 'centered', 
				...
				{data : "codTipoDocumento", sClass: 'hidden'},
				{data : "estadoSeleccion", sClass: 'hidden'}
	...
	function construirTablaFormatos(dataGrilla) {
		...
		$('#tblFormatos').DataTable({
			...
			columns : [
				...
				{data : "codParametro", sClass: 'alignCenter'},
				{data : "desParametro", sClass: 'left'},
				{data : "codParametro", sClass: 'hidden'},
				{data : "estado", sClass: 'hidden'}				
			],
			data: dataGrilla,
            ordering: true,
            searching: false,
	...
	function revalidarFormulario(){
		$("#txtBusqueda").val('');
		filtrar();
		flagPrevisualizacion=false; // Inicio [gangles 26/05/2016] Se agrega la funcionalidad de la previsualización de la plantilla a crear //Fin [gangles 26/05/2016] 
		$('#frmPlantillas').submit();
	}		
	
	function buscar(){
			limpiar("tipoExpediente");
			listaTipDocExp = [];
			var selCodigoTipoExpendiente =  $('#selCodigoTipoExpendiente').val();
			if(selCodigoTipoExpendiente==""){
				return;
			}
			$.ajax({
	...
	function nuevaPlantilla(){
		...
		var desPlantilla =$("#dessPlanReporte").val();
		...
		for (var i = 0; i < dataJson.length; i++) {
			...
			//if($('#'+dataJson[i].codTipoDocumento).prop('checked')){
				...
				obj.DESDOCUMENTO = dataJson[i].desTipoDocumento;
				datos[contador] = obj;
				contador++;
			//}
		}
		...
		/*var formURL = '${pageContext.request.contextPath}/mantPlantRep.htm?action=registrarPlantilla';
		document.forms.frmPlantillas.action = formURL;
		document.forms.frmPlantillas.method = "post";
		document.forms.frmPlantillas.target = "_blank";
		document.forms.frmPlantillas.submit();*/
		$.ajax({				
				url : '${pageContext.request.contextPath}/mantPlantRep.htm?action=registrarPlantilla&desPlantilla='+encodeURIComponent(desPlantilla),
				type : 'POST',
				async : true,
				dataType : "json",
				...
				success : function(response) {
					...
					var mensae = "Se ha creado la plantilla N&deg; "+ codigoPlantilla;
	...
	// Inicio [gangles 26/05/2016] Se agrega la funcionalidad de la previsualización de la plantilla a crear
	function previsualizarPlantilla(){
		...
		//obtenemos los documnetos seleccionados
		var datos = [];
		var contador=0;
	    var dataJson = $('#tblDocumentos').DataTable().rows('.seleccionado').data();	
		for (var i = 0; i < dataJson.length; i++) {
	...
	function filtrar(){
	  ...
	  function limpiar(combo){
		...
		$('#tblDocumentos').dataTable().fnClearTable();
		$('#tblDocumentos').dataTable().fnDraw();
		//$('#frmPlantillas').bootstrapValidator('resetForm', true);
		jQuery('#txtBusqueda').prop('disabled', true);
		if(combo=="proceso")
		{
			$('#selCodigoTipoExpendiente').empty();
			$('#selCodigoTipoExpendiente').val("");
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">NUEVA PLANTILLA DE REPORTE DE EXPEDIENTES DE TRABAJO</h3>
					</div>
				</div>	
				<div class="panel panel-primary">
					<form class="form-horizontal" role="form" name="frmPlantillas" id="frmPlantillas" method="post">
						...
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
								...
								<legend class="scheduler-border"> 3. Reportes Tributario Aduaneros </legend>
									...
									<table id="tblReportes" name="tblReportes" class="table table-striped" style="width: 100%;">
										<thead>
											<tr>
												<th width="5%" class="text-center">N&deg;</th>
												<th width="5%" class="center"><input type="checkbox" id="selectAllReportes"></th><!-- Inicio [gangles 30/05/2016] Se añade el botón marcar y desmarcar todos --><!-- Fin [gangles 30/05/2016]-->
												<th width="10%" class="text-center">Cod. Rubro</th>
												<th width="80%" class="text-center">Descripci&oacute;n del Rubro</th>
												<th></th>
											</tr>
										</thead>
									</table>
							...
							<div class="row content-box">
								...
								<div class="col-md-2">
									<input type='text' class="form-control tamanoMaximo" id='numPlantilla' name='numPlantilla' disabled/>
								</div>
							</div>
							<div class="form-group">
								<div class="col-md-5">
									<Strong>Descripci&oacute;n de la Plantilla de Reporte:</Strong>
									<input type='text' class="form-control tamanoMaximo" id='dessPlanReporte' name='dessPlanReporte' maxlength="100"/>
								</div>	
								<div class="col-md-7">
								</div>
							</div>
							...
							<div class="col-md-7">
									<input type="button" class="btn btn-primary"  id="btnConsulta" intermediateChanges="false"  onClick="revalidarFormulario()" value="Crear Plantilla" id="btnCargarr"></input>									
									<input type="button" class="btn btn-primary"  id="btnPrevisualizar" intermediateChanges="false" onClick="validarFormularioPrevisualizacion()" value="Previsualización Plantilla"></input> <!-- Inicio [gangles 26/05/2016] Se añade el botón para la previsualización de la plantilla a crear --><!-- Fin [gangles 26/05/2016]-->	
