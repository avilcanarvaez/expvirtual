			...
			<div class="row">
				...
				<div class="panel panel-primary">
					<form id="frmAux" name="frmAux" method="post" >
							...
							<input name="hidTxtNumExpedienteVirt" id="hidTxtNumExpedienteVirt" type="hidden" value='${t6614ExpVirtualBean.numExpedienteVirtual}'></input>
					...
					<form name="frmCargaArchivo" id="frmCargaArchivo" method="post" class="form-horizontal">
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
							...
							<div class="form-group">
								<div class="col-md-1"><Strong>N&uacute;mero de RUC:</Strong></div>
								<div class="col-md-2">
...
<!-- 										  <input name="docFisico" id="docFisico" type="file" onchange="validarArchivoUpload()" class="form-control tamanoMaximo uploadStyle" > -->
								...
								<div class="form-group">
									<div class="col-md-12">	
										<p >* Archivos permitidos: PDF, .zip. El archivo .zip solo debe contener archivos Excel (.xls,.xlsx) y/o Archivo de texto .txt. y Archivos de firma(.sig y .crt) .</p>
									</div>
									<div class="col-md-12">	
				...
				<form id="formPrincipalCargaArchivo" class="form-horizontal" method="post" >	
					<div class="col-md-5" align="right" id="dvSecBotones02">	
						...
						<input id="hidTxtNumExpVirt" type="hidden" name="hidTxtNumExpVirt" value='${t6614ExpVirtualBean.numExpedienteVirtual}'></input>
...
// 		 			var url = "${pageContext.request.contextPath}/cargarDocEscritoIt.htm?action=descargarArchivoDoc?codIdentificadorECM=" + tablaDocAdjHid[i].codIdECM +'&numExpedienteVirtual='+numExpVirt+'&nombreArchivo='+tablaDocAdjHid[i].nomArchAdj;
				...
				fileData.numItem = '0';
		 		...
		 		fileData.numExpVirt = numExpVirt;
...
// 		mostrarModal();		
	...
	function volverPaginaReq(){
		...
		var  url = '${pageContext.request.contextPath}/cargarDocumento.htm?action=consultarExpedientesPendientesView';
...
// 		confirmarEnviar();
		...
		titulo = "Detalle de Lista de Archivos";
		if (tabla.length==0) {
	...
	$("#formPrincipalCargaArchivo").submit(function(e){
		...
		formObj.attr('action', '${pageContext.request.contextPath}/cargarDocumento.htm?action=grabarDocAdjDetReq');
				...
				if (data.indexOf("<pre style") > -1) {
					...
					if(metodoInvocar=="obtenerTamano"){						
						...
						if(response.archivoExtensionPermitida == false){ //EXTENSION NO PERMITIDA
							...
							control.val(null);
							$("#docFisico").val("");
							mostrarDlgInfo(titulo, "El tipo de archivo comprimido debe ser: Archivos Excel (.xls,.xlsx) y/o Archivo de texto .txt y Archivos de firma (.sig y .crt)");
						} else if(response.archivoCertSigNoExiste == false){ //FALTA ARCHIVOS DE Firma o Certificado 
							$("#txtNombreArchivo").val("");
							var control = $("#docFisico");
							control.replaceWith( control = control.clone( true ) );
							control.val(null);
							$("#docFisico").val("");
							mostrarDlgInfo(titulo, "Falta archivos .sig o .crt en el archivo adjunto, por favor verificar.");
						} else if(response.archivoPdfA == false){ // VALIDAR ARCHIVO TIPO PDF/A 
							$("#txtNombreArchivo").val("");
							var control = $("#docFisico");
							control.replaceWith( control = control.clone( true ) );
							control.val(null);
						...
						} else if(response.mensajerpta == true){ //MENSAJE ERROR SI NO TIENE FIRMA
							$("#docFisico").val("");
// 							mostrarDlgInfo(titulo, response.mensaje);
							mostrarDlgInfo(titulo, "El archivo no fue firmado correctamente o fue adulterado");
						}else {
							peso = response.peso;
	...
	function agregarListaArchivo(){
		...
		titulo='ESCRITOS ELECTR&Oacute;NICOS'
