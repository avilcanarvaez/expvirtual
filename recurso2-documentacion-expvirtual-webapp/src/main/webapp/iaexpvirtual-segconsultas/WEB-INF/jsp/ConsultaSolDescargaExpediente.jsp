...
<title>RESULTADO DE DESCARGA DE EXPEDIENTES - IU032</title>
<!-- Bootstrap -->
...
<div id="container" class="container" style="width: 95%">
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">EJECUCION DE DESCARGA DE EXPEDIENTES</h3>
						...
						<div class="form-group"><div class="col-md-12"></div></div>							
							...
							<div class="form-group">
								...
								<div class="col-md-2">
									...
									<div class="col-md-1"><Strong> N&deg; Solicitud :</Strong></div>							
								...
								<div class="col-md-12">
									...
									<table id="tablaRepGeneral" class="table display"cellspacing="0" style="width: 100%;"><!--table table-striped table-bordered-->
										<thead>
											<tr>
												<th width="5%">N&#176;</th>
												<th width="15%">N&#176; Solicitud</th>
												<th width="30%">N&uacute;mero de Expediente</th>
												<th width="10%">Fecha de Generaci&oacute;n</th>
												<th width="15%">Estado</th>
												<th width="20%">Usuario Creación</th>
	...
	function consultarReportes(){			
		$('#imprimir').attr('disabled', true);
		$.ajax({
			url : '${pageContext.request.contextPath}/conSegControl.htm?action=cargarListaSolicitudDes',
			type : 'POST',
			async : true,
			dataType : "json",
	...
	function construirTablaPedidos(dataGrilla) {
    	var table = $('#tablaRepGeneral').DataTable({
			...
			columns : [
					...
					{data :	"numSolDescarga",sClass: 'center alinearCentrado',"defaultContent":""},
					{data :	"numExpedienteOrigen",sClass: 'center alinearCentrado',"defaultContent":"" },
					{data :	"fecGeneracionIni",sClass: 'center alinearCentrado',"defaultContent":"" },
					{data :	"descripcionEstSol",sClass: 'center alinearCentrado',"defaultContent":"","sType": "date-eu" },
					{data :	"nombrePersonaRegistro",sClass: 'center alinearCentrado',"defaultContent":"" },
					{data :	"fecGeneracionFin",sClass: 'center alinearCentrado',"defaultContent":"", render : function(data, type,row,meta){
						var aux = row.numSolDescarga;	
						var numExp = row.numExpedienteOrigen;
						var fecIni = row.fecGeneracionIni;
						 var uri = ''; 
						 var link = '';
						 
						 if(row.codEstadoSol=="03" ) {
				             uri = '${pageContext.request.contextPath}/conSegControl.htm?action=descargarSolicitud&numSolDescarga=' +aux
				                 + '&numExpediente='
								+ numExp
								+ '&fecInicio='
								+ fecIni;   
				             link = "<div><a target='_blank' href='" + uri + "'>Terminado</div>";
						 } else if(row.codEstadoSol=="04" ) {
				             uri = '${pageContext.request.contextPath}/conSegControl.htm?action=descargarSolicitud&numSolDescarga=' +aux
				             + '&numExpediente='
								+ numExp
								+ '&fecInicio='
								+ fecIni; 
				             link = "<div><a target='_blank' href='" + uri + "'>Descargado</div>";
						 }else if(row.codEstadoSol=="05" ) {
				             link = "Eliminado";
						 }else {
							 link = 'En Proceso';
						 }
					...
					{data :	"codEstadoSol",sClass: 'hidden',"defaultContent":"" }
				],
				tableTools: {
...
// 					habilitarLink();
	...
	function habilitarLink(){
		...
		var dataJson = $('#tablaRepGeneral').DataTable().rows().data();	
		for (var i = 0; i < dataJson.length; i++) {
			if(dataJson[i].codEstadoSol=="03"){
				document.getElementById(dataJson[i].fecGeneracionFin).style.visibility = 'visible';
			}else{
				document.getElementById(dataJson[i].fecGeneracionFin).style.visibility = 'hidden';
			}
		}
	}
