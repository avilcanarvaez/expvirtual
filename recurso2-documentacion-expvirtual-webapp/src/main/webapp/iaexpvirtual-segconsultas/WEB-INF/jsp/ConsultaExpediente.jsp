...
<title>REGISTRO DE OBSERVACIONES A EXPEDIENTES</title>
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading" align="center">
						<h3 class="panel-title" align="center">REGISTRO DE OBSERVACIONES A EXPEDIENTES</h3>
					...
					<form class="form-horizontal" role="form" name="frmFiltroBusquedaAvanzada" id="frmFiltroBusquedaAvanzada" method="post">
						...
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
							...
							<div class="col-md-12">	
								...
								<div class="form-group" style="margin : 12px 5px !important" id="DocumentoCompleto">
								<div class="border-line panel-scroll">
								<table id="tblExpedientesFiltro" class="table display" cellspacing="0" style="width: 100%;">
									<thead>
										<tr>
											<th width="4%" class="text-center">N&deg;</th>
											...
											<th width="5%" class="text-center">Tipo de Expediente</th>
											<th width="5%" class="text-center">Estado Expediente</th>
											<th width="8%" class="text-center">Fecha Expediente</th>											
											<th width="8%" class="text-center">Origen</th>										
											<th width="8%" class="text-center">Opci&oacute;n</th>											
										</tr>
									</thead>
					...
					<form class="form-horizontal" role="form" name="frmRegistrarObservacion" id="frmRegistrarObservacion" method="post" data-toggle="validator">
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
								...
								<div class="col-md-1" align="right">	
									<button type="button" class="btn btn-primary" intermediateChanges="false" id="btnRegistrar" onclick="revalidarFormularioObservaciones()">Registrar</button>
								</div>	
							</div>
						</fieldset>		
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
					...
					validators: {				
						callback: {
									...
									if(observacion.length<20){
										$('#frmRegistrarObservacion').bootstrapValidator('updateMessage', 'desObservacion', 'callback',  'Debe ingresar como m&iacute;nimo 20 caracteres y como m&aacute;ximo 200 caracteres para el campo Observaci&oacute;n.');
										$('#frmRegistrarObservacion').bootstrapValidator('updateOption', 'desObservacion', 'callback',  'Debe ingresar como m&iacute;nimo 20 caracteres y como m&aacute;ximo 200 caracteres para el campo Observaci&oacute;n.');
										$('#btnRegistrar').attr('disabled', true);
										return false;
									}
...
function cargarTiposExpedientes() {
	...
	if (codProceso == "") {
		...
		$.ajax({
            url: '${pageContext.request.contextPath}/regObsControl.htm?action=cargarListaTiposExpediente',
	...
	function cargaModal(numExpedienteVirtual){
		...
		$.ajax({
			...
			url : '${pageContext.request.contextPath}/regObsControl.htm?action=cargarObservacionesPorExpVirtual',
			type : 'POST',
			async : true,
			dataType : "json",
			...
			success : function(response) {
				...
				var listaObservaciones = JSON.parse(response.listaObservaciones);
				...
				if (listaObservaciones.length > 0) {
					...
					$('#tblObservaciones').dataTable().fnAddData(listaObservaciones); //carga registros en grilla
					$('#exportarObs').attr('disabled', false);
					var titulo = "Registro de Observaciones - N&deg; Expediente Virtual " + datosExpedientes.numExpedienteVirtual;
					mostrarModal(titulo);
				} else {					
					$('#tblObservaciones').dataTable().fnClearTable();
					$('#tblObservaciones').dataTable().fnAddData();
	...
	function consultarObservacionesExpedienteVirtual(){
		var  url = '${pageContext.request.contextPath}/regObsControl.htm?action=cargarObservacionesPorExpVirtual&numExpedienteVirtual='+numExpedienteConsultar;
...
// 	}
	...
	function validarContribuyente(){
		...
		$.ajax({
			...
			url : '${pageContext.request.contextPath}/regObsControl.htm?action=validarContribuyente',
			type : 'POST',
			async : false,
			dataType : "json",
			data : '&numRuc='+numRuc,
	...
	function exportarExcel(){
		...
		if(dataExp.length > 0){
		    ...
		    var formURL = 'regObsControl.htm?action=exportarExcelExpedientes';
		...
		//INICIO LLRB 15012020
		    ...
		    var formURL = 'regObsControl.htm?action=exportarExcelExpedientesFisca';
			document.forms.formPrincipal.action = formURL;
			document.forms.formPrincipal.method = "POST";
			$('#campoHiddenExp').val('');
			$('#campoHiddenExp').val(listaCadena);
	...
	function consultarExpedientes(){
		...
		$.ajax({
			...
			url : '${pageContext.request.contextPath}/regObsControl.htm?action=cargarListadoExpedientesVirtuales',
			type : 'POST',
			async : true,
			dataType : "json",
...
function confirmarRegistrar(){ 
	...
	titulo = 'Registro de Observaciones a Expedientes Virtuales';
	msj = '&iquest;Est&aacute; seguro de registrar la observaci&oacute;n?';
	botones = [];	
...
function registrarObservacion(){
	...
	$.ajax({		
		...
		async : false,
		dataType : "json",
		data : $('#frmRegistrarObservacion').serialize()+'&descripcionObs='+encodeURIComponent(descripcionObs)+'&numExpedienteVirtual='+numExpeVirt,
		//contentType : "application/json",//[jquispe 12/07/2016] Comentado para el envio POST.
		mimeType : "application/json",
		success : function(response) {
			...
			var titulos =  JSON.parse(response.titulos);
			var msjError = response.msjError;
			var avisos =  JSON.parse(response.avisos);
			...
			tituloObsModal = "Registro de Observaciones - N&deg; Expediente Virtual " + datosExpedientes.numExpedienteVirtual;				
			
			mostrarDlgInfo(titulos.tituloDefecto, avisos.aviso2, "segundaCargaModal()");	
...
function exportarObservaciones(){
	var dataObs = $('#tblObservaciones').dataTable().fnGetData();
	//INICIO LLRB 15012020
	var dataExp = $('#tblExpedientesFiltro').dataTable().fnGetData();
	if(dataObs.length > 0){
		if(dataExp.length > 0){
			var listaCadena = JSON.stringify(dataObs);
			var formURL = 'regObsControl.htm?action=exportarExcelObservacionesFisca';
...
function obtenerObservaciones(id){
	var dataEnvio = new Object();
	$('#frmRegistrarObservacion').bootstrapValidator('resetForm', true);			
	...
	if (dataExp.length > 0){
		...
		var dataJson = $('#tblExpedientesFiltro').DataTable().rows().data();	
		for (var i = 0; i < dataJson.length; i++) {
			if(dataJson[i].numOrden==id){
