...
hr {
 ...
 <script type="text/javascript">
    ...
    $(function () {
		...
		//Inicio Lestrada SNADE307-1051  
		$('#tblExpedientes').on('click','.ver-requerimiento',function(e){
			//nchavez [06/06/2016]
			...
			numExpedienteConsultar = numexp;
// 		consultarDetalleExpedienteVirtual();
		cargarDatosBusquedaSession(numexp,nreq,numreqorigen);

		});
		//Fin Lestrada SNADE307-1051  
		inicializarProcesos();
...
// 			console.log(hoverBuscar);
    ...
    function cargarTiposExpedientes() {
		...
		if (codProceso == "") {			
			...
			$.ajax({
	            url: '${pageContext.request.contextPath}/regReqManualControl.htm?action=cargarListaTiposExpediente',
	...
	function construirTablaExpediente(dataGrilla) {
    	/* Inicio jquispe [06/07/2016] Se comento grilla de lista de expediente.
    	...
    	*/
    	...
    	//Inicio jquispe [06/07/2016] Grilla de lista de Requerimiento.
		$('#tblExpedientes').DataTable({
			"language": {
				"url"		:	"/a/js/libs/bootstrap/3.3.2/plugins/datatables-1.10.7/plug-ins/1.10.7/i18n/Spanish.json"
			},
			...
			columns : [
				...
				// 				name: "seleccione",
				// 				id: data,
				// 				onclick: 'guardarNumRequerimiento(this.id)'
				// 			}
				// 		).wrap('<div></div>').parent().html();
				// 	}
				// },
				//Inicio Lestrada SNADE307-1051      	
				{data : "numRequerimOrigen", sClass: 'left alinearCentrado',render:function(data,row,full){
					//return '<a href="#"  onclick="guardarNumExpediente(event,'+full.numExpedienteVirtual+','+full.numRequerimiento+','+full.numRequerimOrigen+')" >'+data+'</a>';
					return '<a href="#" data-id="'+full.numRequerimiento+'"  data-numreqorigen="'+data+'" data-numexp="'+full.numExpedienteVirtual+'" class="ver-requerimiento">'+data+'</a>';
				}},
				//Fin Lestrada SNADE307-1051  
				{data : "numExpedienteVirtual", sClass: 'left alinearCentrado'},
				{data : "numExpedienteOrigen", sClass: 'left alinearCentrado'},
				{data : "numRuc", sClass: 'left alinearCentrado'},
				{data : "desRazonSocial",sClass:'centered alinearCentrado'},
				//Fin [nchavez 31/05/2016]
				{data : "desOrigRequerimiento", sClass: 'left alinearCentrado'},
				{data : "desProceso", sClass: 'left alinearCentrado'},
				{data : "desTipoExpediente", sClass: 'left alinearCentrado'},
				{data : "fecRegistroExpediente", sClass: 'left alinearCentrado'},
				{data : "fechaDocumentoOrigen", sClass: 'left alinearCentrado'},
				...
				{data : "fechaRequerimiento", sClass: 'left alinearCentrado'},
				{data : "fechaVencimiento", sClass: 'left alinearCentrado'}
	...
	function verDetalleDocumento(numexp,nreq,numreqorigen){		
		var  url = '${pageContext.request.contextPath}/regReqManualControl.htm?action=listarDocumentoExpediente&numExpedVirtual='+numexp+'&numRequerimiento='+nreq+'&numRequerimientoOrigen='+numreqorigen;		
		$(location).prop( 'href', url);		
	}
	
	// nchavez [31/05/2016]
	function guardarNumExpediente(e,numExpediente,nreq,numreqorigen){//jquispe [06/07/2016] Agregaron parametros del numero de requerimiento y origen.
		e.preventDefault ? e.preventDefault() : (e.returnValue = false);
		// $('#btnConsulta').prop('disabled', false);
		numExpedienteConsultar = numExpediente;
// 		consultarDetalleExpedienteVirtual();
		cargarDatosBusquedaSession(numExpediente,nreq,numreqorigen);//jquispe [06/07/2016] Agregaron parametros del numero de requerimiento y origen.
	}
	function consultarDetalleExpedienteVirtual(){
		var  url = '${pageContext.request.contextPath}/regReqManualControl.htm?action=consultarDetalleExpedienteVirtualView&numExpedienteVirtual='+numExpedienteConsultar+'&flagPaginaPrincipal=true';
		$(location).prop( 'href', url);
	}
...
// 	}
	...
	function validarContribuyente(){
		...
		$.ajax({
			...
			url : '${pageContext.request.contextPath}/regReqManualControl.htm?action=validarContribuyente',
			type : 'POST',
			async : false,
			dataType : "json",
			data : {'numRuc':numRuc},
	...
	function buscar(){
			...
			$.ajax({
				...
				url : '${pageContext.request.contextPath}/regReqManualControl.htm?action=cargarListadoExpedientesVirtuales',
				type : 'POST',
				async : true,
				dataType : "json",
		...
		function exportar(){
			...
			if(dataExp.length > 0){
				...
				var formURL = 'regReqManualControl.htm?action=exportarExcelExpedientes';
		...
		function cargarDatosBusquedaSession(numexp,nreq,numreqorigen){
					
			$.ajax({
				
				url : '${pageContext.request.contextPath}/regReqManualControl.htm?action=cargarDatosBusquedaSession',
				type : 'POST',
				async : false,
				dataType : "json",
				...
				success : function(response) {
					...
					//consultarDetalleExpedienteVirtual();//[jquispe 06/07/2016] No realiza la consulta a esta pagina.
					verDetalleDocumento(numexp,nreq,numreqorigen);//[jquispe 06/07/2016] Consulta la lista de Documentos.
		...
		function removerDatosBusquedaSession(){
			...
			$.ajax({
				...
				url : '${pageContext.request.contextPath}/regReqManualControl.htm?action=removerDatosBusquedaSession',
				type : 'POST',
				async : false,
				dataType : "json",
				data : '',
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">CONSULTA DE REQUERIMIENTOS</h3>
				...
				<div class="panel panel-primary">
					...
					<form class="form-horizontal" role="form" name="frmFiltroBusquedaAvanzada" id="frmFiltroBusquedaAvanzada" method="post">
						...
						<!-- [jquispe 06/07/2016] La grilla debe de ser igual a la de Atencion de Requerimientos.
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
						...
						<!--	[jquispe 06/07/2016] La grilla debe de ser igual a la de Atencion de Requerimientos.				
												...
												<th width="10%" class="text-center">Fecha del Documento Origen</th>
												<th width="10%" class="text-center">Proceso</th>
												<th width="10%" class="text-center">Tipo de Expediente</th>
												<th width="10%" class="text-center">Fecha de Registro del Expediente</th>
												<th width="15%" class="text-center">Responsable del Expediente</th>
											</tr>
										</thead>
									</table>
									</div>
								...
								<div class="col-md-12">	
									...
									<table id="tblExpedientes" class="table display" cellspacing="0" style="width: 100%;">
										<thead>
											<tr class="active">
												<!-- [nchavez 31/05/2016] -->
												<!-- <th width="8%" class="text-center">Seleccionar Requerimiento</th> -->
												...
												<!-- FIN   [ATORRESCH 20170314] -->
												<th width="8%" class="text-center">Fecha del Requerimiento</th>
												<th width="8%" class="text-center">Fecha de Vencimiento</th>
											</tr>
										</thead>
									</table>
								</div>
