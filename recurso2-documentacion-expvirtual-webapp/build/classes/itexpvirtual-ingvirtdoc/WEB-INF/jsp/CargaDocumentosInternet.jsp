...
<body>
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">INGRESO VIRTUAL DE DOCUMENTOS - ATENCION DE REQUERIMIENTOS</h3>
					</div>
				</div>	
				<div class="panel panel-primary">
					<form id="frmAux" name="frmAux" method="post" >
						...
						<input id="hidTxtTipoOpcion" name="hidTxtTipoOpcion" type="hidden"  class="form-control tamanoMaximo" value='EX' ></input>
					...
					<form name="frmCargaArchivo" id="frmCargaArchivo" method="post" class="form-horizontal">
						...
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
							...
							<div class="form-group">
								...
								<div class="border-line panel-scroll">
									<div>
										<table id="tblTipoDocumentos" name="tblTipoDocumentos"  class="table table-bordered" style="width: 100%;">
											<thead>
												<tr>
													<th width="20%" class="center">Seleccionar Documento</th>
													<th width="30%" class="center">Nombre del Documento</th>	
						...
						<fieldset class="scheduler-border" style="margin : 15px 15px !important">
						<legend class="scheduler-border"> Subir Archivo</legend>
							<div class="form-group">
								...
								<div class="col-md-2" align="right" id="dvSecBotones01">	
									<input type="button" id="btnAdjuntarArchivo" class="btn btn-primary" id="addArchivo" intermediateChanges="false" onClick="agregarListaArchivo()" value="Adjuntar Archivo"></input>
								</div>	
							</div>	
							<div class="form-group">
							</div>	
										...
										<table id="tblDocumentos" name="tblDocumentos"  class="table table-bordered" style="width: 100%;">
											<thead>
												<tr>
													...
													<th width="30%" class="center">Nombre del Documento</th>	
													<th width="37%" class="center">Nombre del Archivo</th>
		...
		$('#frmCargaArchivo').bootstrapValidator({
			...
			fields: {
				txtPalabrasClave: {
                    validators: {
						notEmpty: {
                            message: "<span style='color:red'>(*) Debe Ingresar al menos una Palabra Clave de B&uacute;squeda.</span>"
                        }
                    }
                }
			}	
	...
	//	$('html').block({message: '<h1>Procesando...</h1>'});
		...
		for (var i = 0; i < dataJson.length; i++) {
			var obj = new Object;
			obj.CODDOCUMENTO = dataJson[i][7];
			obj.CODTIPDOC =dataJson[i][8];
			obj.FECDOC =dataJson[i][9];
			obj.MIMETYPE =dataJson[i][10];
			obj.NOMBREARCHIVO =dataJson[i][2];
			//nchavez [06/06/2016]
			obj.MOTIVO=dataJson[i][11];
			//Inicio LEstrada SNADE307-1031
			obj.CORRELATIVO=dataJson[i][0];
			//Fin LEstrada SNADE307-1031
			datos[contador] = obj;
			contador++;
// 			console.log('datos documento',obj);
		}
	...
	function agregarListaArchivo(){
		...
		var data = [numeroFila,desTipoDocumento,nombreArchivo,tamanoArchivo+' MB',verArchivo,opcion,tamanoArchivo,numDocumento,codTipDocumento,fecDocumento,mimeType,motivoDocumento];
	...
	function eliminarFila(fila){
		...
		for (var i = 0; i < dataJson.length; i++) {
			fila=i+1;
			dataJson[i][0]=i+1;
			dataJson[i][5] = "<span name='"+dataJson[i][0]+"' class='glyphicon glyphicon-remove' aria-hidden='true' onclick='confirmarEliminar("+fila+")'></span>"
			var data = [dataJson[i][0],dataJson[i][1],dataJson[i][2],dataJson[i][3],dataJson[i][4],dataJson[i][5],dataJson[i][6],dataJson[i][7],dataJson[i][8],dataJson[i][9],dataJson[i][10]];
	...
	function ObtenerPesoTotal(){
		...
		var dataJson = $('#tblDocumentos').DataTable().rows().data();	
		for (var i = 0; i < dataJson.length; i++) {
			pesoTotal= pesoTotal + parseFloat(dataJson[i][6]);
		}
		return pesoTotal;
	}
	...
	$("#frmAux").submit(function(e){
	    ...
	    formObj.attr('action', '${pageContext.request.contextPath}/cargarDocumentoIt.htm?action='+metodoInvocar); //METODO DEL CONTROLLER
