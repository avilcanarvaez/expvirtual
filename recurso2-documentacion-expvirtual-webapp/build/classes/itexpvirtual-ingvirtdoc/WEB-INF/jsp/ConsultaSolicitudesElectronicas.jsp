...
<script type="text/javascript">
	...
	function construirTablaRequerimiento(dataGrilla) {
		...
		if ($('#selCodigoProceso').val() == "004") {	//Recursos Impugnatorios
			mostrarColumna = "hideColumn";
		}
		...
		$('#tblRequerimientos').DataTable({
			...
			columns : [
				...
				{data : "codTipoExpediente", sClass: "left alinearCentrado " + mostrarColumna},
				{data : "numExpedienteVirtual", sClass: "left alinearCentrado"},
				{data : "numRequerimiento", sClass: "left alinearCentrado " + mostrarColumna},
				{data : "codEstadoExpe", sClass: "left alinearCentrado " + mostrarColumna},
				{data : "numDoc", sClass : "left alinearCentrado " + mostrarColumna,
					render:function(data,row,full) {
						return '<a href="#" onclick="descargarEscrito(\''+data+'\')">Carta de atenci&oacute;n</a>';
					}
				},
				{data : "numExpedienteVirtual", sClass : "left alinearCentrado " + mostrarColumna,
					render:function(data,row,full) {
						return '<a href="#" onclick="consultaRequerimientos(\''+data+'\')">Ver documentos</a>';
					}
				}
			],
			data: dataGrilla,
	...
	function buscar() {
		...
		$.ajax({
			url : '${pageContext.request.contextPath}/solicitudesElectronicas.htm?action=cargarListadoEscritos',
			type : 'POST',
			async : true,
			dataType : "json",
	...
	function exportar() {
		...
		if (dataExp.length > 0) {
			...
			var formURL = 'solicitudesElectronicas.htm?action=exportarExcelEscritosElectronicos';
	...
	function makeInputMask( controlQuery, mask, maxlength, valorInicial ) {
 	...
 	function cargarDatosBusquedaSession() {		
		$.ajax({
			url : '${pageContext.request.contextPath}/solicitudesElectronicas.htm?action=cargarDatosBusquedaSession'+"&tblPage="+getPaginaTbl("tblRequerimientos"),
			type : 'POST',
			async : false,
			dataType : "json",
			data : $('#frmFiltroBusquedaEspecifica').serialize()+'&'+$('#frmFiltroBusquedaAvanzada').serialize(),
	...
	function removerDatosBusquedaSession() {
		$.ajax({
			url : '${pageContext.request.contextPath}/solicitudesElectronicas.htm?action=removerDatosBusquedaSession',
			type : 'POST',
			async : false,
			dataType : "json",
			data : '',
	...
	function descargarEscrito(numEscrito) {
		$('#iPrint').attr('src', "${pageContext.request.contextPath}/solicitudesElectronicas.htm?action=descargarEscrito&numEscrito=" + numEscrito);
		$('#iPrint').attr( 'src', function ( i, val ) { return val; });
	}
	
	function consultaRequerimientos(data) {
		var tabla = $('#tblRequerimientos').dataTable().fnGetData();
		numExpedienteConsultar = tabla[indexDocumentos].numExpedienteVirtual;
		
		cargarDatosBusquedaSession();
		
		var formURL = '/ol-ti-iaexpvirtual-ingvirtdoc/regReqManualControl.htm?action=consultarExpedientesVirtualesView';
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">CONSULTAR SOLICITUD ELECTR&Oacute;NICA</h3>
					</div>
				</div>
				<div class="panel panel-primary">
					...
					<form class="form-horizontal" role="form" name="frmFiltroBusquedaAvanzada" id="frmFiltroBusquedaAvanzada" method="post">
						<fieldset class="scheduler-border"
							...
							<legend class="scheduler-border"> B&uacute;squeda de Solicitudes </legend>
							<div class="form-group">
								<div class="col-md-2">
									<Strong>N&deg; Solicitud:</Strong></Strong>
								</div>
								<div class="col-md-3">
								...
								<div class="col-md-2">
									<Strong>N&deg; Expediente:</Strong></Strong>
								</div>
								<div class="col-md-3">
							...
							<div class="form-group">
								<div class="col-md-3"><Strong>Fecha Solicitud:</Strong></div>
							</div>
							<div class="form-group">
				...
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">LISTA DE SOLICITUDES</h3>
					</div>
				</div>
				<div class="panel panel-primary">
					<fieldset class="scheduler-border" style="margin : 15px 15px !important">
						<legend class="scheduler-border"> Listado de Solicitudes </legend>
							<div class="form-group">
								<div class="col-md-12">
												...
												<table id="tblRequerimientos" class="table display" style="width: 100%;">
													<thead>
														<tr>
															...
															<th width="10%" class="text-center">Fecha Solicitud</th>
															<th width="20%" class="text-center">Tipo de solicitud</th>
															<th width="10%" class="text-center">Nro. Expediente</th>
															<th width="15%" class="text-center">Nro. Requerimiento</th>
															<th width="10%" class="text-center">Estado</th>
															<th width="15%" class="text-center">Carta de atenci&oacute;n</th>
															<th width="10%" class="text-center">Expediente</th>
														</tr>
													</thead>
												</table>
											</div>
