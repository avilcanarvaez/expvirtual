 ...
 <script type="text/javascript">
	...
	$(function () {
		...
		$( "#txtRuc" ).blur(function() {
			...
			//Inicio nchavez 15/06/2016
			if(hoverBuscar){revalidarFormulario();}
			//Fin nchavez 15/06/2016
		});
    ...
    function cargarTiposExpedientes() {
		...
		if (codProceso == "") {			
			...
			$.ajax({
	            url: '${pageContext.request.contextPath}/cargarDocumento.htm?action=cargarListaTiposExpediente',
		...
		$('#tblRequerimientos').DataTable({
			...
			columns : [
				...
				{data : "numRequerimOrigen", sClass: 'left alinearCentrado',render:function(data,row,full){
					return '<a href="#"  onclick="guardarNumRequerimiento(event,'+full.numRequerimiento+')" >'+data+'</a>';
				}},
    ...
    // nchavez [31/05/2016]
	function guardarNumRequerimiento(e,id){
		e.preventDefault ? e.preventDefault() : (e.returnValue = false);
		// $('#btnConsulta').prop('disabled', false);
		var dataJson = $('#tblRequerimientos').DataTable().rows().data();	
			for (var i = 0; i < dataJson.length; i++) {
			if(dataJson[i].numRequerimiento==id){
				
				numExpedienteConsultar = dataJson[i].numExpedienteVirtual;
				numRequerimientoConsultar = dataJson[i].numRequerimiento;
	...
	function cargarDocumentosAlExpediente(){
		var  url = '${pageContext.request.contextPath}/cargarDocumento.htm?action=consultarDetalleRequerimientoView&numExpedienteVirtual='+numExpedienteConsultar+'&numRequerimiento='+numRequerimientoConsultar;
			$.ajax({
				
				url : '${pageContext.request.contextPath}/cargarDocumento.htm?action=cargarDatosBusquedaSession',
				type : 'POST',
				async : false,
				dataType : "json",
...
// 	}
	...
	function buscar(){
			...
			$.ajax({
				//Inicio [jquispe 27/05/2016] Realiza la accion cuando el usuario realiza la consulta.
				url : '${pageContext.request.contextPath}/cargarDocumento.htm?action=cargarListadoRequerimientosPendientes',
				// Fin [jquispe 27/05/2016]
				type : 'POST',
				async : true,
				dataType : "json",
				...
				success : function(response) {
					...
					if (listaRequerimientos.length > 0) {
						...
						//AQ-FSW
						listaRequerimientos = caracterHtml(true,listaRequerimientos);
			...
			<div class="row">
				<div class="panel panel-primary">
					<div class="panel-heading align="center">
						<h3 class="panel-title" align="center">REQUERIMIENTOS PENDIENTES POR ATENDER</h3>
						...
						<fieldset style="margin : 15px 15px !important">
							<div class="form-group">
								<div class="col-md-12" align="center" id="dvSecBotones01">
									<!-- nchavez [27/05/2016] -->
									<!-- <input type="button" class="btn btn-primary"  id="btnConsulta" intermediateChanges="false" data-dismiss="modal" value="Cargar Documentos" id="btnCargarr" onClick="cargarDocumentosAlExpediente()"></input> -->
